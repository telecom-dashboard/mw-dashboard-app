...
function AdminLinkStatusPage() {
  ...
  const fetchRows = async (pageValue = page, pageSizeValue = pageSize, searchValue = search) => {
    try {
      ...
      console.error("Failed to fetch status rows", err);
    ...
    <div style={pageStyle}>
      <div style={headerRow}>
        <div>
          ...
          <p style={subText}>Operational status view for microwave links</p>
        </div>

        <form onSubmit={handleSearch} style={searchWrap}>
          <input
            placeholder="Search link / NE / FE / IP..."
            value={searchInput}
      ...
      <div style={tableCard}>
        <div style={tableHeaderWrap}>
          <h3 style={{ margin: 0 }}>Status Table</h3>
        </div>
...
function SmallCard({ title, value }) {
  return (
    <div style={smallCardStyle}>
      <div style={smallCardTitle}>{title}</div>
...
export default AdminLinkStatusPage;